...
function Experiment (params) {
	...
	this.randomRange = params.randomRange || false;
	...
	if (this.size && this.changeDistance) {
		...
		this.distanceArr = ['450', '300', '150', '600']
	}
	...
	this.colorNumb = (params.colorNumb === undefined) ? false : params.colorNumb;
...
Experiment.prototype.init = function() {
	this.tooltip.hide();
	this.addBallPair(this.colorNumb, this.noDevice);
};
...
Experiment.prototype.addBallPair = function(colorNumb, noDevice) {
	...
	// 4 exp
	if (this.size && this.changeDistance) {
		numb = (colorNumb !== false) ? colorNumb : getRandomInt(0, 3); // случайный цвет
	} else {
		numb = (colorNumb !== false) ? colorNumb : getRandomInt(0, 2); // случайный цвет
	}
	...
	if (numb == 0) { // red
		...
		if (this.randomRange) {
			coord = this.genXYPair(this.distanceArr[getRandomInt(0, 3)])
		} else {
	...
	if (this.noDevice) {
		...
		ball2[0].mouseenter(function () {
			...
			self.addBallPair(self.colorNumb, self.noDevice);
